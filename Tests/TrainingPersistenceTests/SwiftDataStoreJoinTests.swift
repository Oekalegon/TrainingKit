...
@Suite("SwiftDataStore joined activities")
struct SwiftDataStoreJoinTests {
    ...
    func deleteKeepsSharedPiece() async throws {
        ...
        try await store.insertJoinLinkForTesting(joinID: one.id, components: [a.id, b.id])
        try await store.insertJoinLinkForTesting(joinID: two.id, components: [a.id, b.id])
...
extension SwiftDataStore {
    func insertJoinLinkForTesting(joinID: UUID, components: [UUID]) throws {
        modelContext.insert(try ActivityJoinRecord(joinID: joinID, componentIDs: components))
