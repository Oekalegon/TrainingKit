...
- **`PlanReconciler`**: links a completed `Activity` to a `PlannedActivity` (same-day, same-sport, closest duration; a close runner-up is flagged ambiguous rather than silently resolved). Manual link/unlink lives on the Activity Detail sheet.
